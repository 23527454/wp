(function($) {
    ...
    $.fn.bootstrapTreeTable.methods = {
        ...
        // 组件的其他方法也可以进行类似封装........
        expendNode:function(target, parms){
        	  var _ipt = target.find("tbody").find("tr").find("input[name='select_item']:checked");
              // 如果是radio
              if(_ipt.attr("type")=="radio"){
            	
            	  target.find('tr').each(function() {
            		    if ($(this).treegrid("isNode")&&$(this).find("input[name='select_item']:checked").val()==_ipt.val()) {
        ...
        expendNodeByName:function(target, parms){
        	var cssStr = '<span class="glyphicon glyphicon-backward"></span>';
        	target.treegrid("collapseAll");
        	target.find('tr').each(function() {
        		var tree = $(this);
        		$(this).html($(this).html().replace(cssStr,''));
        			$(this).find('td').each(function() {
			    			if($(this).text().indexOf(parms)!=-1){
			    			$(this).html($(this).html().replace($(this).text(),$(this).text()+cssStr));
			    			var parentNode = tree.treegrid("getParentNode");
			    			while(parentNode){
        ...
        selectFirstRow:function(target, parms){
    ...
    $.fn.bootstrapTreeTable.defaults = {
        ...
        toolbar: null,//顶部工具条
        height: 0,
        expanderExpandedClass : 'glyphicon glyphicon-minus-sign',// 展开的按钮的图标
        expanderCollapsedClass : 'glyphicon glyphicon-plus-sign'// 缩起的按钮的图标
