...
var vm = new Vue({
	...
	methods: {
		initheight:function(){
			this.iframeStyle.height=window.innerHeight-130+'px';
		},
		getMenuList: function () {
			$.getJSON(baseURL + "sys/menu/nav", function(r){
                ...
                for(var i=0;i<vm.menuList.length;i++){
        			var menu = vm.menuList[i];
        			if(menu.type==0&&menu.list.length>0){
        				location.href="#"+menu.list[0].url;
        				break;
        			}
        		}
			});
		...
		getUser: function(){
			$.getJSON(baseURL + "sys/user/info", function(r){
				vm.user = r.user;
				vm.logoShow = r.logoShow;
				if(r.alarm.alarmAuth>0&&false){//拥有权限监听
					vm.alarmAuth = true&&hasPermission('alarmEntity:list');
					vm.alarmCount = r.alarm.needRead;
					var timeout = setInterval(function() {
						$.getJSON(baseURL + "sys/user/info", function(r){
							if(r.alarm.alarmAuth>0){//拥有权限监听
								vm.alarmAuth = true&&hasPermission('alarmEntity:list');
								vm.alarmCount = r.alarm.needRead;
							}else{
        ...
        openAlarm:function(){
        	location.href="#modules/equipment/alarmList.html";
        },
	},
	created: function(){
		this.getMenuList();
