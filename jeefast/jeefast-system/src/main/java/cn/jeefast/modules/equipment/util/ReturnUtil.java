...
public class ReturnUtil {
	...
	public static Map<String, Object> getCommunicationData(byte[] dealData) {
		...
		for(int i=0;i<dealData.length;i++) {
			...
			if(i==0) {
				...
				if(b.equals((byte)0x7d)) {
					...
					data = ArrayUtils.add(data, b);
					log.info("接受到的数据："+ByteUtil.bytesToHexString(Arrays.copyOf(dealData, i)));
					break;
				}else {
					data = ArrayUtils.add(data, b);
				}
		...
		log.info("数据格式转换后："+ByteUtil.bytesToHexString(data));
	...
	public static String settingReturn(byte[] data) {
		...
		if (Arrays.equals(dcmd, new byte[] { 0x00, 0x00 })) {
			try {
				...
				throw new RRException("请检查前端机闸机程序版本是否正确，请升级程序版本");
			}
		}
	...
	public static Map<String, Object> returnSearchList(byte[] data) {
		...
		if (returnCode == 0x01) {
			...
			Equipment eq = ByteUtil.bytesToEquipment(Arrays.copyOfRange(equipmentBytes, 0,  36));
			returnMap.put("data", eq);
		}
		return returnMap;
	}
	...
	public static void main(String[] args) {
		returnVersion(ByteUtil.hexStringToBytes("7e90010014000031a2021600524f4e415f5453544c5f4d425f56332e3234455753d000090020190507020f4153d0010900209909090000000042957e"), new Equipment());
	}
	...
	public static Map<String,Object> returnReadStatus(byte[] data){
		...
		List<ReadStatusEntity> readStatusList = new ArrayList<ReadStatusEntity>();
		
		ReadStatusEntity entity = new ReadStatusEntity();
		
		readStatusList.add(entity);
		...
		//长度14  为24对红外线
		if(ByteUtil.bytesToInt(new byte[] {length})==14) {
			String statusBit = ByteUtil.byteArrayToString(Arrays.copyOfRange(data, nowIndex+1, nowIndex+4));
			for(int i=0;i<24;i++) {
				if(i<9) {
					if(statusBit.charAt(i)=='0') {
						sb.append("<img src='../../img/redIco.png'/>L0"+(i+1));
					}else {
						sb.append("<img src='../../img/greenIco.png'/>L0"+(i+1));
					}
				}else {
					if(statusBit.charAt(i)=='0') {
						sb.append("<img src='../../img/redIco.png'/>L"+(i+1));
					}else {
						sb.append("<img src='../../img/greenIco.png'/>L"+(i+1));
					}
				}
			}
			//7对红外线  依然是3个字节
		}else {
			String statusBit = ByteUtil.byteArrayToString(Arrays.copyOfRange(data, nowIndex+1, nowIndex+2));
			for(int i=0;i<8;i++) {
				if(statusBit.charAt(i)=='0') {
					sb.append("<img src='../../img/redIco.png'/>L0"+(i+1));
				}else {
		...
		for(int i=0;i<7;i++) {
			if(i==0) {
				if(hhStatus.charAt(7-i)=='0') {
					sb1.append("<img src='../../img/redIco.png'/>");
				}else {
					sb1.append("<img src='../../img/greenIco.png'/>");
				}
				sb1.append("左到位(从)");
		...
		entity.setHeStatus(sb1.toString());
		
		returnMap.put("data", readStatusList);
