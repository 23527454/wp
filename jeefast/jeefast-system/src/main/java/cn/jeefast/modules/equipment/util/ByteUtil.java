...
public class ByteUtil {
	...
	public static String bytesToMac(byte[] macBytes) {
	 ...
	 public static Equipment bytesToEquipment(byte[] data) {
		 
		 Equipment eq = new Equipment();
		 eq.setAddress(String.valueOf(bytesToInt(new byte[] {data[0]})));
		 eq.setEquipSn(bytesToSn(Arrays.copyOfRange(data, 1, 7)));
		 eq.setIp(binaryArray2Ipv4Address(Arrays.copyOfRange(data, 7, 11)));
		 eq.setPort(bytesToInt(Arrays.copyOfRange(data, 11, 13)));
		 eq.setSubnetMask(binaryArray2Ipv4Address(Arrays.copyOfRange(data, 13, 17)));
		 eq.setGateWay(binaryArray2Ipv4Address(Arrays.copyOfRange(data, 17, 21)));
		 eq.setIpCenter(binaryArray2Ipv4Address(Arrays.copyOfRange(data, 21, 25)));
		 eq.setPortCenter(bytesToInt(Arrays.copyOfRange(data, 25, 27)));
		 eq.setEquipReq(bytesToReq(Arrays.copyOfRange(data, 27, 36)));
		 eq.setOldIp(eq.getIp());
		 eq.setOldPort(eq.getPort());
		 return eq;
	 }
	 ...
	 public static String bytesToTime(byte[] data) {
		 ...
		 String hours = bytesToHexString(Arrays.copyOfRange(data, 4, 5));
		 String minutes = bytesToHexString(Arrays.copyOfRange(data, 5, 6));
		 String second = bytesToHexString(Arrays.copyOfRange(data, 6, 7));
		 StringBuffer sb = new StringBuffer();
