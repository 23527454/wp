...
public class OperateUtil {
	...
	public static byte[] bulidData(byte[] dcmd, byte[] data, byte operateType) {
		...
		log.info("发送数据："+ByteUtil.bytesToHexString(head));
		return head;
	}
	...
	public static byte[] settingBaseInfo(Equipment equip) {
		...
		data = ArrayUtils.addAll(data, ByteUtil.shortToBytes((short) equip.getPort()));
		...
		data = ArrayUtils.addAll(data, ByteUtil.shortToBytes((short) equip.getPortCenter()));
	 ...
	 * 恢复出厂设置
	 */
	public static byte[] settingRestore() {
		byte[] dcmd = new byte[] { (byte) 0xFE, 0x00, 0x01 };
		byte[] data = new byte[] { 0x00 };
		return bulidData(dcmd, data, OperateConstants.operateSetting);
	}
	 ...
	 * 上传记录
	 */
	public static byte[] uploadRecord(Equipment equip) {

		byte[] dcmd = new byte[] { (byte) 0xA2, 0x00, 0x12 };

		byte[] data = {};
		data = ArrayUtils.addAll(data, ByteUtil.SnToBytes(equip.getEquipSn()));
		Date date = equip.getReadRecord().getDate();// yyyy-mm-dd hh:ii:ss
		int week = CommonUtil.weekByDate(date);
		String dateStr = DateUtil.dateToString(date, "yyyyMMddHHmmss");
		data = ArrayUtils.addAll(data,
				ByteUtil.hexStringToBytes(dateStr.substring(0, 8) + "0" + week + dateStr.substring(8)));
		data = ArrayUtils.add(data, ByteUtil.intToByte(Integer.valueOf(equip.getReadRecord().getRemark())));
		data = ArrayUtils.add(data, (byte) 0x00);// 标志 预留字段
		data = ArrayUtils.add(data, ByteUtil.intToByte(Integer.valueOf(equip.getReadRecord().getResource())));
		if (equip.getReadRecord().getRemark().equals("3")) {
			data = ArrayUtils.add(data, ByteUtil.intToByte(Integer.valueOf(equip.getReadRecord().getExceptionInfo())));
		} else {
			data = ArrayUtils.add(data, (byte) 0x00);// 标志 预留字段
		}
		return bulidData(dcmd, data, OperateConstants.operateUpload);
	}

	public static byte[] uploadStatus(Equipment equip) {
		byte[] dcmd = new byte[] { (byte) 0xA2, 0x01, 0x0D};

		byte[] data = {};
		data = ArrayUtils.add(data, ByteUtil.intToByte(Integer.valueOf(equip.getReadStatus().getZjWorkModel())));//运行模式
		//红外状态
		if(equip.getReadStatus().getRedLine()==null) {
			data = ArrayUtils.addAll(data, new byte[] {0x00,0x00,0x00});
		}else {
			StringBuffer redlineBit = new StringBuffer("000000000000000000000000");
			String[] exceptionRedLines = equip.getReadStatus().getRedLine().split(",");
			for(String line : exceptionRedLines) {
				redlineBit.replace(Integer.valueOf(line), Integer.valueOf(line)+1, "1");
			}
			data = ArrayUtils.add(data, ByteUtil.BitToByte(redlineBit.toString().substring(0, 8)));
			data = ArrayUtils.add(data, ByteUtil.BitToByte(redlineBit.toString().substring(8,16)));
			data = ArrayUtils.add(data, ByteUtil.BitToByte(redlineBit.toString().substring(16)));
		}
		//霍尔状态
		if(equip.getReadStatus().getHeStatus()==null) {
			data = ArrayUtils.add(data, (byte)0x00);
		}else {
			StringBuffer heStatusBit= new StringBuffer("00000000");
			String[] exceptionStatus = equip.getReadStatus().getHeStatus().split(",");
			for(String line : exceptionStatus) {
				heStatusBit.replace(Integer.valueOf(line), Integer.valueOf(line)+1, "1");
			}
			data = ArrayUtils.add(data, ByteUtil.BitToByte(heStatusBit.toString()));
		}
		data = ArrayUtils.add(data, ByteUtil.intToByte(Integer.valueOf(equip.getReadStatus().getZzjStatus())));//主闸机状态
		data = ArrayUtils.add(data, ByteUtil.intToByte(Integer.valueOf(equip.getReadStatus().getCzjStatus())));//从闸机状态
		
		data = ArrayUtils.add(data, ByteUtil.intToByte(Integer.valueOf(equip.getReadStatus().getzDriverStatus())));//主驱动状态
		data = ArrayUtils.add(data, ByteUtil.intToByte(Integer.valueOf(equip.getReadStatus().getcDriverStatus())));//从驱动状态
		
		data = ArrayUtils.add(data, ByteUtil.intToByte(Integer.valueOf(equip.getReadStatus().getZjCrossStatus())));//通行状态
		data = ArrayUtils.add(data, ByteUtil.intToByte(Integer.valueOf(equip.getReadStatus().getKzqStatus())));//通行状态
		data = ArrayUtils.addAll(data, ByteUtil.shortToBytes(Short.valueOf(equip.getReadStatus().getPersonTotal())));//通行状态
	 ...
	 * @return
	 */
	public static byte[] settingEquipTime(Equipment equip) {
		byte[] dcmd = new byte[] { (byte) 0xD0, 0x00, 0x08};

		byte[] data = {};
		
		data = ArrayUtils.addAll(data, ByteUtil.hexStringToBytes(String.valueOf(equip.getYear())));
		data = ArrayUtils.add(data, ByteUtil.intToByte(equip.getMonth()));
		data = ArrayUtils.add(data, ByteUtil.intToByte(equip.getDay()));
		data = ArrayUtils.add(data, ByteUtil.intToByte(equip.getWeek()));
		data = ArrayUtils.add(data, ByteUtil.intToByte(equip.getHours()));
		data = ArrayUtils.add(data, ByteUtil.intToByte(equip.getMinute()));
		data = ArrayUtils.add(data, ByteUtil.intToByte(equip.getSecond()));
	 ...
	 * @return
	 */
	public static byte[] settingEquipValidTime(Equipment equip) {
		byte[] dcmd = new byte[] { (byte) 0xD0, 0x01, 0x04};

		byte[] data = {};
		
		data = ArrayUtils.addAll(data, ByteUtil.hexStringToBytes(String.valueOf(equip.getValidYear())));
		data = ArrayUtils.add(data, ByteUtil.intToByte(equip.getValidMonth()));
		data = ArrayUtils.add(data, ByteUtil.intToByte(equip.getValidDay()));
