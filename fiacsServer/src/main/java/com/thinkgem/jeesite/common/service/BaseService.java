...
//							if (!user.getOffice().getArea().getId().equals("")) {
//								sqlString.append(" OR " + oa + ".id = '" + user.getOffice().getArea().getId() + "'");
//								sqlString.append(
//										" OR " + oa + ".parent_ids LIKE '" + user.getOffice().getArea().getParentIds()
...
//						// 区域及以下数据
							...
							if (user.getOffice().getArea().getParent().getParentId().equals("0")) {
								String pids = user.getOffice().getParentIds()+user.getOffice().getId();
								if(!"1".equals(user.getCpmpFlag())) {
								sqlString.append(" OR " + oa + ".id in ("
										+ " select id from sys_area area where area.id='"+user.getOffice().getArea().getId()+"' union select id from sys_area areap where areap.id in(select area.parent_id from sys_area area where area.id = '"+user.getOffice().getArea().getId()+"')" 
										+ ")"
										);
								}
...
//						// 网点及以下数据
							...
							if (!user.getOffice().getId().equals("")) {
								...
								if(oa=="ar"&&("3".equals(r.getId())||"2".equals(r.getId()))) {
									oa="a";
								}
								if("3".equals(r.getId())) {
									sqlString.append(" OR " + oa + ".id = '" + user.getOffice().getId() + "'");
...
//							/*
//							 * 网点是string 区域是int 选择所属区域时如果没有区域 就再搜索网点
//							 * 
//							 **/
//							if (!user.getOffice().getId().equals("")) {
//								sqlString.append(" OR " + oa + ".id = '" + user.getOffice().getId() + "'");
...
//								}
//							} else {
//								for (String oa1 : StringUtils.split(officeAlias, ",")) {
//									// sqlString.append(" OR " + oa + ".id = " +
//									// user.getOffice().getId());
//									sqlString.append(" OR " + oa1 + ".id IS NULL");
//								}
//							}
//						}
			...
			/*
			 * if (!isDataScopeAll) { if (StringUtils.isNotBlank(userAlias)) {
			 * for (String ua : StringUtils.split(userAlias, ",")) {
			 * sqlString.append(" OR " + ua + ".id = '" + user.getId() + "'"); }
...
//						}
	...
	public static String dataScopeFilter(Staff staff, String officeAlias, String userAlias) {
		User user = staff.getCurrentUser();
		StringBuilder sqlString = new StringBuilder();
...
//						}
	...
	public static String dataScopeFilterArea(User user, String officeAlias, String userAlias) {
...
//						}
						...
						// 网点及以下数据
							...
							sqlString.append(" OR " + oa + ".id = '" + user.getOffice().getArea().getId() + "'");
						//	sqlString.append(" OR " + oa + ".id = '" + user.getOffice().getArea().getParent().getId() + "'");
						}
//						// 当前网点数据
//						else if (Role.DATA_SCOPE_OFFICE.equals(r.getDataScope())) {
//							sqlString.append(" OR " + oa + ".id = '" + user.getOffice().getArea().getId() + "'");
