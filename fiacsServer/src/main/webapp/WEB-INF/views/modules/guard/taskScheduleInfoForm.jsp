...
<script type="text/javascript">
	    ...
	    var dataStaff = taskPersonInfoList;
        			...
        			for(var i=0;i<dataStaff.length;i++){
        			    if(dataStaff[i].workStatus==0){
        			    alert("班组存在已离职的人员，不可用！");
        			    return false;
        			    }
        			    if(dataStaff[i].workStatus==2){
                        alert("班组存在正在休假的人员，不可用！");
                        return false;
                        }
                        if(dataStaff[i].endDate<new Date()){
                        alert("班组存在已过有效期的人员，不可用！");
                          return false;
                        }
        			}
        				for(var i=0;i<dataStaff.length;i++){
		...
		function setBZMessage() {
			...
			$.getJSON("${ctx}/guard/classTaskInfo/ajaxGetClassTaskInfo?id="+claID,function(data){
                                if(initLists(data.classCarInfoList, data.classPersonInfoList, data.linNodesList)){
		...
		function myModalCar() {
	 	...
	 	function myModalCar2() {
			...
			var name = $.trim(carName);
				var dgCar;				
				dgCar = $('#dgCar').datagrid({
					url : "${ctx}/guard/classTaskInfo/listData?id="+id+"&car.name="+name, // JSON数据路径
					loadMsg : "正在努力加载数据，请稍后...", // 遮罩层
					async : true,
					fit : false,// 自动大小
					fitColumns : false, // 是否固定列
		...
		function myModalStaff() {
	 	 ...
	 	 function myModalStaff2() {
	 		var id=$("#classTaskfo_id").val();
			var keyName = $("#keyName").val();
			var name = $.trim(keyName);
			 	var dgStaff;
				dgStaff = $('#dgStaff').datagrid({
					url : "${ctx}/guard/classTaskInfo/listData?id="+id+"&staffTwo.name="+name, // JSON数据路径
					loadMsg : "正在努力加载数据，请稍后...", // 遮罩层
					async : true,
					fit : false,// 自动大小
					fitColumns : false, // 是否固定列
...
<div class="container-fluid">
	<ul class="nav nav-tabs">
		...
		<li class="active"><a
			href="${ctx}/guard/taskScheduleInfo/form?id=${taskScheduleInfo.id}&selectedClassTaskId=${selectedClassTaskId}&www=0">排班信息<shiro:hasPermission
					name="guard:taskScheduleInfo:edit">${not empty taskScheduleInfo.id?'查看':'添加'}</shiro:hasPermission>
				<shiro:lacksPermission name="guard:taskScheduleInfo:edit">添加</shiro:lacksPermission></a></li>
	</ul>
	<br />
	<form:form id="inputForm" modelAttribute="taskScheduleInfo"
		action="${ctx}/guard/taskScheduleInfo/save" method="post"
		class="form-horizontal">
		<form:hidden path="id"  id="primaryKey"/>
		...
		<div class="row">
			<div class="col-xs-7">
				...
				<div class="form-group">
					...
					<c:choose>
						<c:when test="${not empty taskScheduleInfo.id}">
					<div class="col-xs-5">
						<div class="input-group">
							<form:input path="classTaskInfo.name" htmlEscape="false"
								id="classTaskfo_name"  readonly="true"
			...
			<c:otherwise>
				...
				<div class="form-group">
					...
					<div class="colgetInterNumInt-xs-8">
...
<!-- 							class="btn btn-success btn-sm control-label">线路信息</button> -->
		...
		<div class="form-group">
			...
			<%-- <c:if test="${taskScheduleInfo.id==''&&taskScheduleInfo.id==null}">
				<input id="btnSubmit" class="btn btn-primary" type="submit"
						value="保 存" />&nbsp;
				<input id="btnCancel" class="btn" type="button" value="返 回"
					onclick="history.go(-1)" />
	...
	<div class="modal fade" id="myModalCar" tabindex="-1" role="dialog"
		...
		<div class="modal-dialog" style="width: 650px;">
			<div class="modal-content">
				...
				<input type="button" value="搜索" onclick="myModalCar2()"/>
	...
	<div class="modal fade" id="myModalStaff" tabindex="-1" role="dialog"
		...
		<div class="modal-dialog" style="width: 650px;">
			<div class="modal-content">
				...
				<input type="button" value="搜索" onclick="myModalStaff2()"/>
