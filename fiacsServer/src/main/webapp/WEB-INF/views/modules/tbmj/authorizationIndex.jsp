	...
	<div id="content" class="row-fluid">
		<div id="left" class="accordion-group">
			<div class="accordion-heading">
				<a class="accordion-toggle">网点列表<i
			...
			<input htmlEscape="false" id="office_id"
				...
				htmlEscape="false" id="office_name" class="input-xlarge required"
				style="display: none" /> <input htmlEscape="false" id="office_type"
				class="input-xlarge required" style="display: none" />
		</div>
		<div id="openClose" class="close">&nbsp;</div>
		<div id="right">
			<iframe id="officeContent" src="${ctx}/tbmj/authorization/list"
	...
	var tree,setting = {
			...
			callback : {
				onClick : function(event, treeId, treeNode) {
					...
					$('#officeContent').attr(
							"src",
							"${ctx}/tbmj/authorization/list?accessParaInfoId="+ id.substring(1));
					$('#office_id').val(id);
					$('#office_name').val(treeNode.name);
					$('#office_type').val(treeNode.type);
				}
			}
		};

		function refreshTree() {
			$.getJSON("${ctx}/sys/office/treezeeData2", function(data) {
