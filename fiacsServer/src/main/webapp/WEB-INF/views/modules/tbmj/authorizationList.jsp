...
<head>
	...
	<script type="text/javascript">
								...
								top.$.jBox.confirm("确认要导出门禁权限数据吗？", "系统提示", function(
									...
									if (v == "ok") {
										$("#searchForm").attr("action",
												"${ctx}/tbmj/authorization/export");
										$("#searchForm").submit();
										$("#searchForm").attr("action",
												"${ctx}/tbmj/authorization/list");
									}
								}, {
									buttonsFocus : 1
								});
							...
							function() {
								...
								if(accessParaInfoId==null || accessParaInfoId==""){
									...
									top.$.jBox.confirm("确认要同步该门的数据吗？", "系统提示", function(
										...
										if (v == "ok") {
											$("#searchForm").attr("action",
													"${ctx}/tbmj/authorization/download");
											$("#searchForm").submit();
											$("#searchForm").attr("action",
													"${ctx}/tbmj/authorization/list");
										}
		...
		function page(n, s) {
			$("#pageNo").val(n);
			$("#pageSize").val(s);
			$("#searchForm").attr("action", "${ctx}/tbmj/authorization/list");
			$("#searchForm").submit();
			return false;
		}
	</script>
...
<table id="contentTable"
	...
	<tr>
		...
		<th>人员门禁分组</th>
		<th>允许查库</th>
		<shiro:hasPermission name="tbmj:authorization:edit"><th>操作</th></shiro:hasPermission>
	</tr>
	</thead>
	<tbody>
	<c:if test="${page.list!=null}">
		<c:forEach items="${page.list}" var="a" varStatus="varStatus">
			<tr>
				...
				<td>
					<c:forEach items="${fns:getDictList('permission_group')}" var="type">
						<c:if test="${type.value eq a.permissionGroup}">${type.label}</c:if>
					</c:forEach>
				</td>
				<td>
				...
				<shiro:hasPermission name="tbmj:authorization:edit"><td>
					<a href="${ctx}/tbmj/authorization/form?id=${a.id}&accessParaInfoId=${accessParaInfoId}">修改</a>
					<a href="${ctx}/tbmj/authorization/delete?id=${a.id}">删除</a>
