...
<body>
<form class="layui-form" method="post" id="addForm" action="${ctx}/tbmj/workdayParaInfo/save" style="margin-right: 10%;margin-top: 10%">
    ...
    $(function() {
        $("#getCommit").on('click',function () {
            ...
            if(start==null || start=="" || end==null || end==""){
                ...
                var start=$("#start").val();
                var end=$("#end").val();
                $.post("${ctx}/tbmj/workdayParaInfo/save?eId=${eId}&start="+start+"&end="+end,function (data) {
                    if(data){
                        alert("设置成功！");
        ...
        $("#addRest").on('click',function () {
            layer.open({
                ...
                btnAlign: 'c',
                yes: function(){
                    $.post("${ctx}/tbmj/workdayParaInfo/insertDefault?eId=${eId}",function (data) {
                        if(data){
                            alert("设置成功！");
                            parent.location.reload()
                        }else{
