...
<ul class="nav nav-tabs">
	<li class="active"><a href="${ctx}/tbmj/downloadTimezoneInfo/">门禁参数同步列表</a></li>
</ul>
<form:form id="searchForm" modelAttribute="downloadTimezoneInfo"
		   action="${ctx}/tbmj/downloadTimezoneInfo/" method="post"
...
<table id="contentTable"
	...
	<c:forEach items="${page.list}" var="downloadTimezoneInfo" varStatus="varStatus">
		<tr is-download="${downloadTimezoneInfo.isDownload}">
			...
			<td>${fns:getDictLabel(downloadTimezoneInfo.accessParaInfo.doorPos, 'door_pos', '')}</td>
			<td>${fns:getDictLabel(downloadTimezoneInfo.timezoneInfo.timeZoneType, 'time_zone_type', '')}</td>
			<td>${fns:getDictLabel(downloadTimezoneInfo.accessParaInfo.timeZoneNum, 'time_zone_num', '')}</td>
			...
			<td>
				<shiro:hasPermission name="tbmj:downloadTimezoneInfo:edit">
					<c:if test="${downloadTimezoneInfo.isDownload == '0'}">
						<a href="${ctx}/tbmj/downloadTimezoneInfo/delete?id=${downloadTimezoneInfo.id}"
						   onclick="return confirmx('确认要取消该同步记录吗？', this.href)">取消</a>
					</c:if>
				</shiro:hasPermission>
			</td>
