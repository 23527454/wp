						...
						top.$.jBox.confirm("确认要导出防区参数数据吗？", "系统提示", function(
							...
							if (v == "ok") {
								$("#searchForm").attr("action",
										"${ctx}/tbmj/defenseParaInfo/export");
								$("#searchForm").submit();
								$("#searchForm").attr("action",
										"${ctx}/tbmj/defenseParaInfo/list");
							}
						}, {
							buttonsFocus : 1
						});
					...
					function() {
						...
						if(eid==null || eid==""){
							...
							top.$.jBox.confirm("确认要同步该设备的防区数据吗？", "系统提示", function(
								...
								if (v == "ok") {
									$("#searchForm").attr("action",
											"${ctx}/tbmj/defenseParaInfo/download");
									$("#searchForm").submit();
									$("#searchForm").attr("action",
											"${ctx}/tbmj/defenseParaInfo/list");
								}
...
<ul class="nav nav-tabs">
	<li class="active"><a href="${ctx}/tbmj/defenseParaInfo/">防区信息列表</a></li>
</ul>
...
<table id="contentTable" class="table table-striped table-bordered table-condensed">
	...
	<tr>
		...
		<th>防区属性</th>
		<th>报警延时时间（秒）</th>
		<shiro:hasPermission name="tbmj:defenseParaInfo:edit"><th>操作</th></shiro:hasPermission>
	</tr>
	</thead>
	<tbody>
	<c:if test="${list!=null}">
		<c:forEach items="${list}" var="defenseParaInfo" varStatus="aStatus">
			<tr>
				...
				<shiro:hasPermission name="tbmj:defenseParaInfo:edit"><td>
					<a href="${ctx}/tbmj/defenseParaInfo/form?id=${defenseParaInfo.id}">修改</a>
