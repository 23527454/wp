...
<ul class="nav nav-tabs">
	<li class="active"><a href="${ctx}/tbmj/downloadWorkdayParaInfo/">门禁参数同步列表</a></li>
</ul>
<form:form id="searchForm" modelAttribute="downloadWorkdayParaInfo"
		   action="${ctx}/tbmj/downloadWorkdayParaInfo/" method="post"
...
<table id="contentTable"
	...
	<c:forEach items="${page.list}" var="downloadWorkdayParaInfo" varStatus="varStatus">
		<tr is-download="${downloadWorkdayParaInfo.isDownload}">
			...
			<td>
				<shiro:hasPermission name="tbmj:downloadWorkdayParaInfo:edit">
					<c:if test="${downloadWorkdayParaInfo.isDownload == '0'}">
						<a href="${ctx}/tbmj/downloadWorkdayParaInfo/delete?id=${downloadWorkdayParaInfo.id}"
						   onclick="return confirmx('确认要取消该同步记录吗？', this.href)">取消</a>
					</c:if>
				</shiro:hasPermission>
			</td>
