...
<ul class="nav nav-tabs">
	<li class="active"><a href="${ctx}/tbmj/downloadDefenseParaInfo/">防区参数同步列表</a></li>
</ul>
<form:form id="searchForm" modelAttribute="downloadDefenseParaInfo"
		   action="${ctx}/tbmj/downloadDefenseParaInfo/" method="post"
...
<table id="contentTable"
	...
	<c:forEach items="${page.list}" var="downloadDefenseParaInfo" varStatus="varStatus">
		<tr is-download="${downloadDefenseParaInfo.isDownload}">
			...
			<td>
				<shiro:hasPermission name="tbmj:downloadDefenseParaInfo:edit">
					<c:if test="${downloadDefenseParaInfo.isDownload == '0'}">
						<a href="${ctx}/tbmj/downloadDefenseParaInfo/delete?id=${downloadDefenseParaInfo.id}"
						   onclick="return confirmx('确认要取消该同步记录吗？', this.href)">取消</a>
					</c:if>
				</shiro:hasPermission>
			</td>
